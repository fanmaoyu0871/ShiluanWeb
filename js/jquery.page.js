...
(function($){
	...
	function Page(container,fn,args,style){
		...
		var style=this.style=$.extend({
			...
			"paddL":0,//左边留白
			"paddR":0,//右边留白
			"borderColor":"#437FBA",//边线颜色
			"currentColor":"white",//当前页码的字体颜色
			"disableColor":"#437FBA",//不可点击按钮的字体颜色
			"disableBackColor":"white",//不可点击按钮的背景色
		...
		this.obj.css({'content':'','display':'inline-block','clear':'both'});
		//初始化
		this.init();
	}
	...
	Page.prototype.fillHtml=function(){
		...
		if(args.showPrev){
			if(args.current > 1){
				...
				obj.remove('.prevPage');
				obj.append('<span class="disabled" style="border-top-left-radius:5px; border-bottom-left-radius:5px;">上一页</span>');
			}
		}
		//中间页码
		if(args.current != 1){
		...
		if(args.showNext){
			if(args.current== args.pageCount||args.pageCount==1){
				obj.remove('.nextPage');
				obj.append('<span class="disabled" style="border-top-right-radius:5px; border-bottom-right-radius:5px;">下一页</span>');
			}else{
	...
	Page.prototype.setStyle=function(){
		...
		this.obj.find('a.prevPage').css({'width':s.prevNextWidth, 'color':s.borderColor, 'font-size':s.fontSize+'px','height':s.height-2+'px','line-height':s.height-2+'px', 'border-top-left-radius':5+'px', 'border-bottom-left-radius':5+'px'});
		this.obj.find('a.nextPage').css({'width':s.prevNextWidth, 'color':s.borderColor, 'font-size':s.fontSize+'px','height':s.height-2+'px','line-height':s.height-2+'px', 'border-top-right-radius':5+'px', 'border-bottom-right-radius':5+'px'});
		...
		this.obj.find('span.current').css({'display':'inline-block', 'background-color':s.borderColor,'height':s.height+'px','line-height':s.height+'px','color':s.currentColor,'vertical-align':'middle','width':s.pageWidth+'px'});
		...
		this.obj.find('.tcdNumber').css({'width':s.pageWidth-2+'px','height':s.height-2+'px','line-height':s.height-2+'px', 'color':s.borderColor});
